...
object YtDlpRuntime {
    ...
    @Synchronized
    fun refreshIfDue(context: Context, force: Boolean = false): Boolean {
        val prefs = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
        val lastSuccess = prefs.getLong(LAST_SUCCESS, 0L)
        val now = System.currentTimeMillis()

        if (!force && lastSuccess > 0L && now - lastSuccess < CHECK_INTERVAL_MS) {
            return true
        }
        ...
        return try {
            ...
            Log.w("YtDlpRuntime", "Could not refresh yt-dlp; keeping current runtime", t)
            false
        }
    }
}
